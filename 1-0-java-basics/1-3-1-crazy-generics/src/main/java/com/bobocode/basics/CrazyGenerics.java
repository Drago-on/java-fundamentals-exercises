...
public class CrazyGenerics {
    ...
    public interface Converter <T,R>{ // todo: introduce type parameters
        R convert(T source); // todo: add convert method
    }
    ...
    static class CollectionUtil {
        ...
        public static <T extends BaseEntity> boolean hasDuplicates(List<T> entities, T targetEntity) {
            return entities.stream().filter(entity -> entity.getId().equals(targetEntity.getId())).count() > 1; // todo: update method signature and implement it
        }
        ...
        public static void swap(List<?> elements, int i, int j) {
            ...
            Collections.swap(elements, i, j); // todo: complete method implementation
        }
    }
}
